...
const options = {
  ...
  callbacks: {
     ...
     * @param  {object} _user         User object    (if using database sessions)
     *                               JSON Web Token (if not using database sessions)
     * @return {object}              Session that will be returned to the client
     */
    session: async (session: object, _user: object): Promise<object> => {
     ...
     * @param  {object}  _account   Provider account (only available on sign in)
     * @param  {object}  _profile   Provider profile (only available on sign in)
     * @param  {boolean} _isNewUser True if new user (only available on sign in)
     * @return {object}            JSON Web Token that will be saved
     */
    jwt: async (token, _user, _account, _profile, _isNewUser) => {
